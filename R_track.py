...
#    dt['pl_ur']=(dt['mp']-dt['lp'])*dt['lc']*100  #dead trade->lc=0
    dt['pl_ur']=(dt['mp']-dt['ip'])*dt['lc']*100  #lp is break_even price
    ...
    dt['var_hd']=dt['delta']*dt['atr_hd']
    dt['var_hv']=dt['vega']*dt['atr_hv']*100
    dt['atr_id']=dt['close']*dt['iv']/(252**0.5)
    dt['var_id']=dt['delta']*dt['atr_id']
...
#        to_sql_delete("delete from tbl_c where exit_dt <>'N'")
    ...
    if dt.empty:
        ...
        return
#    elif len(dt.columns)>len(set(dt.columns)):   #chekc if duplicate columns
#        print("track_raw: ln373, dt has dupe columns")
#        return
    else:
...
#reporting
    ...
    col_rpt=['v_spy', 'var_id', 'var_hv', 'vega', 'weigh', 'pl_ur', 'pl_r','mar','wrtn_22_pct']  
    dt[col_rpt]=dt[col_rpt].replace('',0)      
    rpt_1=dt.pivot_table(index=['exp_dt','sec'], values= col_rpt, aggfunc= 'sum', \
...
#    rpt_1=pd.DataFrame(rpt_1.to_records())
    ...
    rpt_2.drop_duplicates(keep='first', inplace=True)
    show_rpt_2=['sec','v_spy','var_id','weigh','wrtn_22_pct',\
                'srtn_22_pct', 'pl_ur','pl_r','vega','mar']
...
#    con_hv=dt.play.isin(play_hv)
    ...
    CON_spike= (dt['spike']>spike_min) |(dt['spike_etd']>spike_min)
#    con_hv_rankup=dt['hv_rank']>dt['i_hv_rank']*1.1
#    con_hv_rankdn=dt['hv_rank']<dt['i_hv_rank']*0.9
    con_hiv= con_hiv_up | con_hiv_dn
    CON_spike= (np.abs(dt['spike'])>spike_min) |(np.abs(dt['spike_etd'])>spike_min)
...
#    con_size=(dt['size_pct']>1) | ((dt['pl']<0) & dt['size_pct']>loss_size_pct_max) \
#        |((dt['pl']>0) & (dt['size_pct']< gain_size_pct_min) & (dt['days_pct']<size_chg_days_pct_max))
#    CON_size= con_weigh |con_size
    CON_tgt_p= (dt['days_pct']>0.2) & (dt['tgt_p']==0)
    con_add=(dt['pl_pct']>0) & (dt['days_pct']<size_chg_days_pct_max) & (dt['size_pct']<gain_size_pct_min)
    con_cut=(dt['pl_pct']<0) & (dt['days_pct']>=size_chg_days_pct_max) & (dt['size_pct']>=loss_size_pct_max)
    con_cut=con_cut | con_weigh
...
#clean display
    ...
    show_alerts=['ticker','risk_live', 'pl_pct', 'pl_ur','var_id','size_buff','prob_t','delta',\
                 'a_out', 'a_stop', 'a_exit', 'a_size', 'a_be', 'a_dv', 'a_spike', 'a_itm' \
                 ,'a_event', 'a_momt','a_unop', 'a_key']
    show_base=['ticker','risk_live','pl_pct','days_pct', 'prob_t', 'prob','lp', 'mp','tgt_p','close','beta','si']
    
    show_stop=show_base + ['bedn','beup']
    show_out=show_base 
    show_exit=show_base + ['days_to_exp']
    show_size=['ticker','a_size','pl_pct','size_pct', 'weigh','size_buff','mp','days_pct','var_id', 'var_hd', 'risky','play']
    ...
    show_spike=['ticker','var_id', 'spike','spike_etd','delta','close','entry_p', 'iv', 'i_iv', 'i_hv_22', 'i_hv_rank', 'hv_rank','pl_pct','days_pct']
    show_itm=['ticker','days_pct','close', 'strike', 'div_dt', 'lsnv','pc','play']
    show_unop=['ticker','mc','bc']
    show_event= ['ticker','days_pct','close','div_dt','event_dt', 'earn_dt']
    show_momt=['ticker', 'entry_p','strike','close', 'delta', 'play','sec','rtn_22_chg','srtn_22_chg']
    ...
    print("\n ---- SIZE ---   \n ", dt[CON_size][show_size].sort_values(['pl_pct','size_pct'], ascending=[True, False]))
    print("\n ---- BE ---   \n ", dt[CON_be][show_be])
