...
#ax8: dist_s & stat_s
    ...
    bs= (df['buck_d'].value_counts(normalize=True))  
...
#    ax11.table(cellText=df_sig_d.values, \
#              rowLabels=df_sig_d.index,\
#              colLabels= df_sig_d.columns,\
#              loc='bottom')
...
#    plt.setp(ax.get_xticklabels(),visible=False)
    ...
    y_loc=ax_close.max()
    ax.annotate('x_dt: %s'%x_dt, xy=(x_dt, y_loc), xytext=(15, 15), \
                textcoords='offset points') #, arrowprops=dict(arrowstyle='-|>'))
 #   style = dict(size=10, color='gray')
#    ax.text('x_dt', 270, 'x_dt', ha='right', **style)
    plt.axvline(x_dt)
#    plt.ion()
#    plt.show()    
#    ax.clear()
