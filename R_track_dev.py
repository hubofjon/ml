...
def track_data(q_date):  #sp500 and etf trade in one table
    from R_stat import stat_VIEW
    from R_options import get_option_simple
    
    #fmt='{:0.1f}'
#    if underlying=='sp500':
    q_trade_hist="SELECT * FROM tb_trade WHERE exit_date<>'N'" #get ride of non-live trade
    q_trade="SELECT * FROM tb_trade WHERE exit_date ='N'"   #get live trade
# save historical trades
    dh=read_sql(q_trade_hist, q_date)
    if set(['level_0']).issubset(dh.columns):    
        dh=dh.drop(['level_0'],1)
    to_sql_append(dh, 'tb_trade_hist')  #save expired trade to tb_trade_hist
...
def mc_CANDY():
    ...
    show_candy=['ticker','play','ip', 'mc','bc', 'fm_hi','fm_lo','fm_50','fm_200']
    generic_candy=['acct','comm', 'sec','exp_date','div_date','pl',\
        'note','delta','vega',\
        'ip1','ic1','ip2','ic2','iiv','beup','bedn',\
        'op1','oc1','op2','oc2','op','oiv','tp','tdate',\
        'a_be','a_div','a_exp','a_vol']
...
#    if underlying=='sp500':
    ...
    q_trade="SELECT * FROM tb_trade WHERE o_date ='N'"   #get live trade
# save historical trades
    dh=read_sql(q_trade_hist, q_date)
    if set(['level_0']).issubset(dh.columns):    
        dh=dh.drop(['level_0'],1)
...
#trd_entry: ticker, sec, exp_dt, div_dt, earn_dt, i_srtn_22__pct, i_rtn_22_pct, i_rsi
#cont: i_mc (concat), i_bc, fm_hi, fm_lo, fm_50, fm_200, entry_p
#add_filed: acct, termc, play (L,S,N), comm, strike,\
...
## alert condition
    ...
    con_v=stk vol chg |pd.isnotnull(dt['mc']) |pd.isnotnull(dt['mc'])
#    con_o=pd.isnotnull(dt['mc']) |pd.isnotnull(dt['mc'])
    ...
    con_m= (con_l & (dt['srtn_22_pct']<dt['i_strn_22_pct']|dt['rtn_22_pct']<dt['i_trn_22_pct']\
                    | dt['rsi']<dt['i_rsi'])) \
        |(con_s & (dt['srtn_22_pct']>dt['i_strn_22_pct']|dt['rtn_22_pct']>dt['i_trn_22_pct']\
                   | dt['rsi']<dt['i_rsi']))
