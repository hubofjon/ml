...
# UPDATE RAW from stat_view, len(dt)=len(ds) !!!       
    ...
    dt['date']=q_date
    dt['close']=ds.loc[con, 'close'].tolist()  #?? FOR all tickers??
...
#raw fileds to update daily
    raw=['close','fm_hi','fm_lo','fm_50', 'fm_200','rsi','srtn_22_pct', 'rtn_22_pct',\
         'mc','bc']
    for f in raw:
        dt.loc[loc_t,'s%'%f]=ds.loc[loc_p,'s%'%f].tolist()
...
# UPDATE derived:   
    ...
    dt['pl_pct']=(dt['pl_r']+dt['pl_ur'])/dt['risk_orig']-1
...
## UPDATE ALERT
    ...
    con_s=dt['play']=='S'
    con_n=dt['play']=='N'
    con_momt_l= con_l & (  (dt['srtn_22_pct']<dt['i_strn_22_pct']) \
                    | (dt['rtn_22_pct']<dt['i_rtn_22_pct']) \
                    | (dt['rsi']<dt['i_rsi'])  ) 
    con_momt_s= con_s & ( (dt['srtn_22_pct']>dt['i_strn_22_pct']) \
                   | (dt['rtn_22_pct']>dt['i_rtn_22_pct']) \
                   | (dt['rsi']<dt['i_rsi']) )
...
#    con_v=stk vol chg |pd.notnull(dt['mc']) |pd.notnull(dt['mc'])
    CON_ov=pd.isnotnull(dt['mc']) |pd.isnotnull(dt['bc'])
...
#UPDATE alert
    ...
    dt['cap']=df_cap.loc[dt['acct'].isin(df_cap['acct']), df_cap['cap']]
    dt['weigh']=dt.groupby('acct')['risk_live']/(dt['cap'])
    dt['buff']=1- dt.groupby('acct')['risk_live'].sum()/dt['cap']
    ...
    show=['acct','ticker','ip', 'lp', 'mp', 'risk_live','pl_pct', 'days_to_exp']
    show_alerts=['a_stop','a_be','a_exit','a_run','a_key', 'a_momt',\
               'a_event','a_ov', 'a_weigh','a_buff']
