...
#    df['spike_d'].plot(kind='hist',xticks=buckets, ax=ax7)
    ...
    ax7.table(cellText=df_stat_d.values, \
...
#    df['spike_s'].plot(kind='hist',xticks=buckets, ax=ax8)
    ax8=plot_blend(df['spike_d'], buckets, ax8, 'orange')
    ax8.table(cellText=df_stat_s.values, \
...
def norm_test(serie, ax):
    ...
    result=p_value>0.05
    title_norm="log_rtn norm?: %s, p_value: %s"%(result, p_value)
