...
def plot_pv(q_date, df_pv_t, candy_t=pd.DataFrame(), mode=''):
    #dt: single ticker
    import matplotlib.pyplot as plt
    ...
    dt=df_pv_t.sort_values('date', ascending=True)
...
#CRITICAL: as index is not continuous from tbl_pv_all for single ticker
    ...
    if candy_t.empty:
        plt.title("%s, norm?:%s, last_spk >%s_std: %s \n spk: %s -  %s - %s"\
                  %(ticker, norm_result, sig, last_spike_d, d_25, d_50, d_75))  
    else:
        candy_t['norm']=norm_result
        candy_t['last_%s'%sig]=last_spike_d
        candy_t['d_25']=d_25
        candy_t['d_50']=d_50
        candy_t['d_75']=d_75
        plt.title(candy_t)
...
def plot_pvs(q_date, ticks='', candy=pd.DataFrame()): #ticks =['x','y'], do(unop)
    ...
    for t in ticks:
        df_pv_t=dp[dp.ticker == t]
        candy_t=candy
        if not candy.empty:
            candy_t=candy[candy['ticker']==t]
        plot_pv(q_date, df_pv_t, candy_t)
