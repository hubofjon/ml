...
#    from R_options import get_option_simple
    ...
    dt_hist=read_sql(q_trade_hist, q_date)
#    if set(['level_0']).issubset(dh.columns):    
#        dh=dh.drop(['level_0'],1)

# APPEND non-live trd to tbl_trd_hist
    to_sql_append(dt_hist, 'tbl_c_hist') 
    
# REPLACE live trade to tbl_trd   
    dt_live=read_sql(q_trade, q_date)  
    dt_live=dt.fillna(0)
    con_nullticker=pd.isnull(dt_live.ticker)
    i_nullticker=dt_live[con_nullticker].index
    dt_live=dt_live.drop(i_nullticker)
    to_sql_replace(dt_live, "tbl_c")  
...
# UPDATE from external
    ...
    if dt['mp']=='' | pd.isnull(dt['mp']): # if mp is NOT updated by tbl_trd@Flask
        dt['mp']=dt['lp']  
...
#REPLACE tbl_track with latet data    
    dt_track_live=dt[dt.ticker.isin(dt_live.ticker)]
...
def old_track_show(dt ):  #risk monitor parameters
 #   to_sql_replace(dt[tbl_trade_view], 'tb_trade')
