...
from P_intel import marketbeat, get_ta, get_earning_date, get_sis_sqz, get_earnings_ec
from T_stat_rework_v1 import stat_run_rework
capital=200000
...
def trade_track_rework(underlying, reval_date):
    start_time=timer()
    df=track_data_prep(underlying, reval_date)
...
# add bechmark       
        if underlying =='sp500':
           try:
#               x,y,z=get_tsm(ticker)
               ds=stat_run_rework(reval_date,ticker)
               x,y,z=ds['sec'].values[0], ds['rtn_22_pct'].values[0],\
                   ds['srtn_22_pct'].values[0]
           except:
               print(colored("tsm error:  %s"%ticker, 'red', 'on_cyan'))
               x,y,z=0,0,0
           dt.loc[index,'sec']=x
